---
title: 自动驾驶规划算法 - RRT、RRTStar 等基本原理
date: 2021-04-13 12:00:00
---
# 自动驾驶规划算法 - RRT、RRTStar 等基本原理
***
...
- **改进的方法**：人为引导随机点的采样
  ...
  - 目标概率 `p` 对随机树拓展的影响
    - `p` 越小，树的分支也就越多（过度随机采样），这会导致生长缺乏方向性，是一种「碰运气」式的搜索
    - `p` 越大，产生分支的太少（过度趋向目标采样，类似最佳优先的贪心搜索），会导致很难绕过障碍物找到目标点
...
![](https://pic3.zhimg.com/80/v2-fe979d17ca84018c6459a1e7a520ba0a_720w.jpg)
