...
可以把 end-to-end 理解为一个函数黑盒，由输入数据直接得到输出数据，下面是一个自然语言处理问题中传统机器学习和端到端深度学习的比较（图片来自 CSDN @[ViatorSun](https://me.csdn.net/ViatorSun)）：
...
图像卷积要利用一个二维滤波器矩阵（卷积前进行 180 度翻转），称为卷积核，用该卷积核在二维图像上滑动，并计算卷积核和对应领域的乘积作为一次卷积结果，重复这个过程，完成二维图像的卷积，LeetCode 上有[图像卷积](https://leetcode-cn.com/problems/image-smoother/)的算法题。
